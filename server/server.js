...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    await sendEmailWithRealFallback({
      to: email,
      subject: 'Mã OTP xác thực tài khoản BonBonCar 🔑',
      body: `
        ...
        <div style="font-size: 24px; font-weight: bold; color: #009698; background-color: #f3f4f6; border: 1px solid #e5e7eb; padding: 12px; border-radius: 8px; text-align: center; letter-spacing: 6px; margin: 20px 0; max-width: 200px; margin-left: auto; margin-right: auto;">
          ${emailVerificationToken}
        </div>
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (!user.isEmailVerified) {
      ...
      await sendEmailWithRealFallback({
        to: user.email,
        subject: 'Gửi lại: Mã OTP xác thực tài khoản BonBonCar 🔄',
        body: `
