      ...
      activeCars.forEach(car => {
        ...
        if (carBookings.length > 0) {
          scheduleStr = carBookings.map(b => {
            const start = new Date(b.pickupDate).toLocaleDateString('vi-VN');
            const end = new Date(b.returnDate).toLocaleDateString('vi-VN');
        ...
        carsContext += `- Xe ID ${car.id}: ${car.brand} ${car.model} (${car.seats} chỗ, truyền động ${car.transmission}, nhiên liệu ${car.fuel}, khu vực ${car.location}, Biển số ${car.plateNumber}, Giá ${car.pricePerDay.toLocaleString('vi-VN')} VND/ngày). Lịch đã bận: ${scheduleStr}.\n`;
      });
    } catch (dbError) {
