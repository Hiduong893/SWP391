  ...
  create: async (voucherData) => {
    ...
    try {
      ...
      const res = await p.request()
        ...
        .input('max_discount_amount', sql.Decimal(18,2), voucherData.maxDiscountAmount ? voucherData.maxDiscountAmount : null)
        ...
        .input('start_date', sql.DateTime2, voucherData.startDate ? new Date(voucherData.startDate) : new Date())
        .input('expiration_date', sql.DateTime2, voucherData.expirationDate ? new Date(voucherData.expirationDate) : null)
        .query(`
          INSERT INTO Vouchers (code, discount_percent, max_discount_amount, max_usage, target_user, target_car_name, start_date, expiration_date)
          OUTPUT inserted.*
          VALUES (@code, @discount_percent, @max_discount_amount, @max_usage, @target_user, @target_car_name, @start_date, @expiration_date)
        `);
  ...
  getActiveVouchers: async () => {
    try {
      ...
      const res = await p.request().query(`
        ...
        WHERE status = 'active'
          AND (start_date IS NULL OR CAST(start_date AS DATE) <= CAST(GETDATE() AS DATE))
          AND (expiration_date IS NULL OR CAST(expiration_date AS DATE) >= CAST(GETDATE() AS DATE))
          AND (max_usage IS NULL OR current_usage < max_usage)
        ORDER BY created_at DESC
      `);
      return res.recordset;
    } catch (e) {
      console.error('voucherModel getActiveVouchers error:', e);
      return [];
    }
  },
